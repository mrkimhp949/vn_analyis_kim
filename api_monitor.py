...
class APIMonitor:
    ...
    def _save_health_data(self):
        """Lưu health data"""
        data = {
            endpoint: asdict(health) for endpoint, health in self.health_data.items()
        }
    ...
    def ping(
            ...
            try:
                ...
                if method.upper() == "GET":
                    ...
                    response = requests.request(
                        method, endpoint, params=params, timeout=timeout
                    )
        ...
        failure_rate = (
            (health.total_failures / health.total_requests * 100)
    ...
    def get_status_summary(self) -> Dict:
        ...
        healthy = sum(
            1 for h in self.health_data.values() if h.status == APIStatus.HEALTHY.value
        )
        degraded = sum(
            1 for h in self.health_data.values() if h.status == APIStatus.DEGRADED.value
        )
        down = sum(
            1 for h in self.health_data.values() if h.status == APIStatus.DOWN.value
        )
