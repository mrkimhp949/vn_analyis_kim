...
class ConservativePositionSizer:
    ...
    def calculate_position_size(
        ...
        if available_capital <= 0:
            ...
            return self._zero_position(
                f"Exposure đã đạt limit ({current_exposure:,.0f} VNĐ)"
            )
            ...
            try:
                ...
                if vol_forecaster.load():
                    ...
                    if predicted_vol > 0.05:
                        ...
                        warnings.append(
                            f"⚠️ Giảm position 30% do volatility cao ({predicted_vol*100:.1f}%)"
                        )
        ...
        if risk_percent > self.max_risk_per_trade * 100:
            ...
            warnings.append(
                f"⚠️ Đã giảm shares để risk <= {self.max_risk_per_trade*100}%"
            )
    ...
    def _calculate_dca_entries(self, base_price: float, total_shares: int) -> list:
        """
    ...
    def print_portfolio_status(self):
        ...
        print(
            f"📊 Đã đầu tư:      {status['invested']:>20,.0f} VNĐ ({status['exposure_percent']:.1f}%)"
        )
    ...
    def format_position_message(
        self, symbol: str, position: PositionSize, entry_price: float
    ) -> str:
