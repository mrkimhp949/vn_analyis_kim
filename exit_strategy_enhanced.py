...
class EnhancedExitStrategy(ImprovedExitStrategy):
    ...
    def __init__(self,
                 ...
                 breakeven_activation: float = 0.10):  # Activate breakeven after 10% profit
        super().__init__(
        ...
        self.dynamic_trailing_stops = {}  # {symbol: float} - current trailing stop price
    ...
    def check_exit(self,
                   ...
                   partial_exits: List[float] = None) -> ExitDecision:
        """
        ...
        if self.use_breakeven_stop and pnl_percent >= self.breakeven_activation * 100:
            if symbol not in self.breakeven_activated:
                ...
                logger.info(f"✅ Breakeven stop activated for {symbol} at {pnl_percent:.1f}% profit")
        ...
        if self.use_dynamic_trailing and pnl_percent >= self.trailing_activation * 100:
            ...
            trailing_distance = self._calculate_dynamic_trailing_distance(df, pnl_percent)
            
            # Update trailing stop
            if symbol not in self.dynamic_trailing_stops:
                self.dynamic_trailing_stops[symbol] = highest_price * (1 - trailing_distance)
            else:
                # Only move trailing stop up, never down
                new_trailing = highest_price * (1 - trailing_distance)
                self.dynamic_trailing_stops[symbol] = max(
                    self.dynamic_trailing_stops[symbol],
                    new_trailing
                )
    ...
    def _calculate_dynamic_trailing_distance(self, df: pd.DataFrame, pnl_percent: float) -> float:
        """
    ...
    def _check_take_profit_levels_enhanced(self,
                                          current_price: float,
    ...
    def record_partial_exit(self, symbol: str, exit_price: float, 
                           shares_exited: int, remaining_shares: int,
                           entry_price: float, exit_reason: ExitReason):
        ...
        logger.info(f"📝 Recorded partial exit for {symbol}: {shares_exited} shares @ {exit_price:,.0f}")
