...
def main():
    ...
    if choice == "1":
        symbol = input(f"Nhập mã cổ phiếu (mặc định: {TICKERS[0] if TICKERS else 'VNM'}): ").strip().upper()
        if not symbol:
    ...
    # === OPTION 3: Threshold Comparison ===
        ...
        if comparison_results:
            ...
            print("\n" + "=" * 80)
            print(f"🏆 TỐT NHẤT THEO RETURN: {best_return['Threshold']}% ({best_return['Return (%)']:.2f}%)")
            print(f"🏆 TỐT NHẤT THEO SHARPE: {best_sharpe['Threshold']}% (Sharpe: {best_sharpe['Sharpe']:.2f})")
            print("=" * 80)
