...
class TestRiskManagementService:
    ...
    def risk_service(self):
        ...
        with patch('services.risk_service.get_circuit_breaker'), \
             patch('services.risk_service.get_emergency_stop'), \
...
class TestEntrySignalService:
    ...
    def entry_service(self):
        ...
        with patch('services.entry_service.EnhancedMLSignalGenerator'), \
    ...
    async def test_scan_single_ticker_success(self, entry_service):
        ...
        with patch('services.entry_service.load_data') as mock_load, \
             patch('services.entry_service.DataValidator'):
...
class TestExitManagementService:
    ...
    def exit_service(self):
        ...
        with patch('services.exit_service.ImprovedExitStrategy'), \
             patch('services.exit_service.EnhancedMLSignalGenerator'), \
             patch('services.exit_service.get_portfolio_manager'), \
             patch('services.exit_service.get_paper_account'):
    ...
    async def test_check_single_position_exit(self, exit_service):
        ...
        with patch('services.exit_service.load_data') as mock_load, \
             patch('services.exit_service.DataValidator'):
...
class TestNotificationService:
    ...
    async def test_send_risk_alert(self, notification_service):
        ...
        await notification_service.send_risk_alert(
            ...
            message="Max loss per day reached"
        )
