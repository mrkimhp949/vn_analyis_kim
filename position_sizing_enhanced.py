...
class EnhancedPositionSizer:
    ...
    def calculate_position_size(self,
                               ...
                               avg_win_loss_ratio: Optional[float] = None) -> EnhancedPositionSize:
        """
        ...
        if portfolio_risk is not None and portfolio_risk >= self.max_portfolio_risk:
            raise RiskManagementError(
                ...
                context={'portfolio_risk': portfolio_risk, 'limit': self.max_portfolio_risk}
            )
        ...
        if sector:
            ...
            if sector_exp >= self.max_sector_exposure:
                raise RiskManagementError(
                    ...
                    context={'sector': sector, 'exposure': sector_exp, 'limit': self.max_sector_exposure}
                )
        ...
        if available_capital <= 0:
            return self._zero_position(
                f"Exposure limit reached ({current_exposure:,.0f} VNĐ)",
                warnings
            )
