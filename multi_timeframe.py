...
class MultiTimeframeAnalyzer:
    ...
    def analyze_multi_timeframe(self, symbol):
        ...
        try:
            ...
            print(
                f"  📅 Daily: {daily_signal['signal']} ({daily_signal['confidence']}%)"
            )
        ...
        try:
            ...
            print(
                f"  📆 Weekly: {weekly_signal['signal']} ({weekly_signal['confidence']}%)"
            )
    ...
    def analyze_quick(self, symbol):
        ...
        try:
            ...
            weekly_trend = (
                "UP" if weekly_ema20.iloc[-1] > weekly_ema50.iloc[-1] else "DOWN"
            )
    ...
    def _combine_signals(self, signals):
        ...
        combined_score = (
            daily_score * weights["daily"] + weekly_score * weights["weekly"]
        )

        # Confidence calculation
        if daily_signal == weekly_signal:
            # Cùng hướng -> confidence cao
            confidence = daily_conf * weights["daily"] + weekly_conf * weights["weekly"]
