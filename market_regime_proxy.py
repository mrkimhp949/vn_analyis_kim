...
class ProxyMarketRegimeAnalyzer:
    def __init__(self):
        ...
        safe_print(f"🔧 Khởi tạo Market Regime Analyzer với {len(self.proxy_stocks)} mã từ config")
    
    def analyze_market_regime(self):
        from config import TICKERS
        safe_print(f"📈 PHÂN TÍCH THỊ TRƯỜNG: Bắt đầu phân tích {len(self.proxy_stocks)} mã")
