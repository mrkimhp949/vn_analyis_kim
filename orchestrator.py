...
class TradingOrchestrator:
    ...
    def adjust_signal_with_news(self, entry_signal, news_context):
        ...
        if entry_signal.should_enter and self.entry_logic and entry_signal.confidence < self.entry_logic.min_confidence:
            entry_signal.should_enter = False
    ...
    def format_entry_recommendation(self, symbol, entry_signal, position, market_regime, news_context=None):
    ...
    async def run_scan(self, market_regime: Dict, vnindex_df: Optional[pd.DataFrame]):
        ...
        if not all([self.entry_logic, self.position_sizer, self.exit_strategy]):
            logging.critical("❌ Các chiến lược (entry, sizing, exit) chưa được thiết lập cho Orchestrator.")
            return
        ...
        logging.info(f"📊 Đang nắm giữ: {len(existing_symbols)} mã ({', '.join(existing_symbols) if existing_symbols else 'không có'})")
    ...
    async def check_active_positions(self, market_regime: Dict, vnindex_df: Optional[pd.DataFrame]):
    ...
    async def _check_single_position(self, symbol: str, pos_data: Dict, market_regime: Dict, vnindex_df: Optional[pd.DataFrame]):
    ...
    async def scan_for_new_entries(self, current_tickers, existing_symbols, market_regime, vnindex_df):
                ...
                if entry_result:
                    if entry_result['type'] == 'signal':
                        async with results_lock:
                            ...
                            if symbol in existing_symbols:
                                if self.portfolio_lock: self.portfolio_lock.cancel_position(symbol)
                                return
                        ...
                        if success:
                            ...
                            signal_count += 1
                            logging.info(f"✅ {symbol}: {entry_result['signal'].signal_type} ({entry_result['signal'].confidence}%)")
                            await self.bot.send_message(self.chat_id, entry_result['message'], parse_mode='Markdown')
                        else:
                            logging.error(f"❌ Paper trade failed for {symbol}: {paper_msg}")
    ...
    async def process_single_ticker_for_entry(self, symbol, df, market_regime, vnindex_df):
        ...
        entry_signal = self.entry_logic.analyze_entry(df=df, ml_signal=ml_signal, market_regime=market_regime)
        ...
        if not entry_signal.should_enter:
            ...
            if self.entry_logic and confidence_for_watchlist >= max(0, self.entry_logic.min_confidence - 5):
        ...
        position = self.position_sizer.calculate_position_size(
            symbol=symbol, entry_price=price, stop_loss=entry_signal.stop_loss,
            confidence=entry_signal.confidence, signal_strength=entry_signal.strength.name,
            market_regime=market_regime
        )
        ...
        can_add, lock_reason = self.portfolio_lock.can_add_position(
            symbol=symbol, position_value=position.shares * price,
            total_capital=self.position_sizer.total_capital, current_positions=self.portfolio_manager.get_positions()
        )
        if not can_add:
        ...
        msg = self.format_entry_recommendation(symbol, entry_signal, position, market_regime, news_context)
        return {
            'type': 'signal',
            'signal': entry_signal,
            'position': position,
    ...
    def execute_paper_buy(self, entry_result):
        ...
        success, paper_msg, _ = self.paper_account.execute_buy(
            symbol=symbol, shares=position.shares, price=entry_signal.entry_price,
            signal_confidence=entry_signal.confidence,
            signal_reason=", ".join(entry_signal.reasons[:2]),
            stop_loss=entry_signal.stop_loss,
            take_profit=position.recommended_entries[-1]['price'] if position.recommended_entries else None
        )
        
        if success:
            if self.position_sizer:
                self.position_sizer.add_position(symbol, position.shares, entry_signal.entry_price)
    ...
    async def perform_post_scan_risk_analysis(self):
        ...
        try:
            risk_positions = {
                sym: {'shares': pos.get('shares', 0), 'avg_price': pos.get('avg_price', 0),
            ...
            if risk_metrics.risk_status in ['HIGH', 'CRITICAL']:
                ...
                await self.bot.send_message(self.chat_id, f"⚠️ *PORTFOLIO RISK ALERT*\n\n{risk_summary}", parse_mode='Markdown')
        except Exception as e:
            logging.error(f"Lỗi portfolio risk analysis: {e}", exc_info=True)

    async def send_summary_and_watchlist(self, num_scanned, signal_count, market_regime, watchlist_candidates):
        """Gửi tóm tắt và danh sách theo dõi."""
        regime_text = market_regime.get('regime', 'UNKNOWN')
        summary = f"✅ Hoàn thành quét {num_scanned} mã\n" \
        ...
        if signal_count == 0:
            if watchlist_candidates:
                ...
                lines = [
                    "• {symbol}: {conf:.0f}% - {reason}{sentiment}{headline}".format(
                        symbol=item['symbol'], conf=item['confidence'], reason=item['reason'],
            ...
            await self.bot.send_message(self.chat_id, text=watchlist_msg, parse_mode='Markdown')