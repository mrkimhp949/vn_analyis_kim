...
class TradingDB:
    ...
    def create_tables(self):
        ...
        with self.get_connection() as conn:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_portfolio_date ON portfolio_history(date)')
    ...
    def save_position(self, symbol: str, shares: int, avg_price: float, 
                     entry_date: str, entry_value: float, 
        ...
        with self.get_connection() as conn:
            ...
            ''', (symbol, shares, avg_price, entry_date, entry_value, stop_loss, take_profit,
    ...
    def save_portfolio_snapshot(self, date: str, total_value: float, total_cost: float,
                               pnl: float, pnl_percent: float, num_positions: int,
                               metadata: Optional[Dict] = None):
        ...
        with self.get_connection() as conn:
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (date, total_value, total_cost, pnl, pnl_percent, num_positions,
                  json.dumps(metadata) if metadata else None))
    ...
    def save_trade(self, symbol: str, action: str, shares: int, price: float,
                  total_value: float, trade_date: str, reason: Optional[str] = None,
                  metadata: Optional[Dict] = None):
        """Save a trade"""
        with self.get_connection() as conn:
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (symbol, action, shares, price, total_value, trade_date, reason,
                  json.dumps(metadata) if metadata else None))
