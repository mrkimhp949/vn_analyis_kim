...
class EnhancedRiskManager(RiskManager):
    ...
    def calculate_enhanced_position_size(self, symbol, price, atr, confidence, signal='BUY', market_volatility=0.02):
        """
    ...
    def _calculate_market_regime_factor(self):
        ...
        try:
            ...
            if len(vnindex_data) > 20:
                market_trend = (vnindex_data['close'].iloc[-1] / vnindex_data['close'].iloc[-20] - 1)
    ...
    def suggest_enhanced_limit_orders(self, current_price, atr, signal='BUY', confidence=50):
        ...
        if signal == 'BUY':
            return {
                'aggressive': round(current_price - (atr * 0.3 * confidence_adjustment), -2),
                'moderate': round(current_price - (atr * 0.5 * confidence_adjustment), -2),
                'conservative': round(current_price - (atr * 0.7 * confidence_adjustment), -2),
