...
class MLModelMonitor:
    ...
    def __init__(self,
                 ...
                 retrain_trigger_days: int = 7):  # Retrain if accuracy drops for 7 days
    ...
    def _save_predictions(self):
        ...
        try:
            with open(self.predictions_file, 'w', encoding='utf-8') as f:
                json.dump({'predictions': self.predictions}, f, indent=2, ensure_ascii=False)
        except Exception as e:
    ...
    def calculate_metrics(self, model_version: str = "default") -> ModelMetrics:
        ...
        incorrect_predictions = [p for p in model_predictions if not p.get('correct', False)]
        
        avg_confidence_correct = np.mean([p['predicted_confidence'] for p in correct_predictions]) if correct_predictions else 0
        avg_confidence_incorrect = np.mean([p['predicted_confidence'] for p in incorrect_predictions]) if incorrect_predictions else 0
    ...
    def calibrate_confidence(self, raw_confidence: float, model_version: str = "default") -> float:
        """
        ...
        calibration_factor = metrics.accuracy / (metrics.avg_confidence / 100.0) if metrics.avg_confidence > 0 else 1.0
        calibration_factor = max(0.5, min(calibration_factor, 1.5))  # Clamp between 0.5x and 1.5x
    ...
    def should_retrain(self, model_version: str = "default") -> Tuple[bool, str]:
        ...
        if metrics.accuracy < self.accuracy_threshold:
            reasons.append(f"Accuracy {metrics.accuracy:.1%} < threshold {self.accuracy_threshold:.1%}")
        
        # Check 2: Calibration error too high
        if metrics.calibration_error > self.calibration_error_threshold:
            reasons.append(f"Calibration error {metrics.calibration_error:.1%} > threshold {self.calibration_error_threshold:.1%}")
        ...
        recent_predictions = [
            ...
            and (datetime.now() - datetime.fromisoformat(p['date'])).days <= self.retrain_trigger_days
        ]
        
        if len(recent_predictions) >= 20:  # Need at least 20 recent predictions
            recent_correct = sum(1 for p in recent_predictions if p.get('correct', False))
            recent_accuracy = recent_correct / len(recent_predictions)
            
            if recent_accuracy < metrics.accuracy * 0.8:  # 20% drop in accuracy
                reasons.append(f"Recent accuracy {recent_accuracy:.1%} dropped significantly from {metrics.accuracy:.1%}")
    ...
    def get_performance_summary(self, model_version: str = "default") -> str:
        ...
        lines.append(f"✅ Avg Confidence (Correct): {metrics.avg_confidence_correct:.1f}%")
        lines.append(f"❌ Avg Confidence (Incorrect): {metrics.avg_confidence_incorrect:.1f}%")
