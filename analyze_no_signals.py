...
class NoSignalAnalyzer:
    ...
    def format_report(self, results: Dict) -> str:
        ...
        lines.append(f"• BUY signals: {ml['buy_signals']} ({ml['buy_signals']/ml['total']*100:.1f}%)")
        lines.append(f"• SELL signals: {ml['sell_signals']} ({ml['sell_signals']/ml['total']*100:.1f}%)")
        lines.append(f"• HOLD signals: {ml['hold_signals']} ({ml['hold_signals']/ml['total']*100:.1f}%)")
        lines.append("")
        ...
        lines.append(f"• ✅ Passed all: {filters['passed_all']} ({filters['passed_all']/total*100:.1f}%)")
        lines.append(f"• ❌ Failed confidence: {filters['failed_confidence']} ({filters['failed_confidence']/total*100:.1f}%)")
        lines.append(f"• ❌ Failed trend: {filters['failed_trend']} ({filters['failed_trend']/total*100:.1f}%)")
        lines.append(f"• ❌ Failed support/resistance: {filters['failed_support']} ({filters['failed_support']/total*100:.1f}%)")
        lines.append(f"• ❌ Failed volume: {filters['failed_volume']} ({filters['failed_volume']/total*100:.1f}%)")
        lines.append(f"• ❌ Failed volatility: {filters['failed_volatility']} ({filters['failed_volatility']/total*100:.1f}%)")
        lines.append(f"• ❌ Failed RSI: {filters['failed_rsi']} ({filters['failed_rsi']/total*100:.1f}%)")
        lines.append("")
