...
class ParallelScanner:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    results.append(
                        ScanResult(symbol=symbol, success=False, error="Timeout")
                    )
                except Exception as e:
                    logger.error(f"Error scanning {symbol}: {e}")
                    results.append(
                        ScanResult(symbol=symbol, success=False, error=str(e))
                    )
    ...
    def _scan_single(self, symbol: str, scan_function: Callable) -> ScanResult:
        ...
        try:
            ...
            return ScanResult(symbol=symbol, success=True, data=data, duration=duration)
        except Exception as e:
            duration = time.time() - start_time
            logger.error(f"Error scanning {symbol}: {e}")

            return ScanResult(
                symbol=symbol, success=False, error=str(e), duration=duration
            )
...
class MLSignalScanner(ParallelScanner):
    ...
    def scan_for_signals(
        self, symbols: List[str], min_confidence: float = 60.0
    ) -> List[Dict]:
        """
...
class EntrySignalScanner(ParallelScanner):
    ...
    def __init__(self, ml_generator, entry_logic, market_regime, max_workers: int = 5):
        super().__init__(max_workers=max_workers)
        self.ml_generator = ml_generator
    ...
    def scan_for_entries(
        self, symbols: List[str], existing_symbols: set = None
    ) -> List[Dict]:
        """
        ...
        def scan_function(symbol: str) -> Dict:
            ...
            entry_signal = self.entry_logic.analyze_entry(
                df=df, ml_signal=ml_signal, market_regime=self.market_regime
            )
