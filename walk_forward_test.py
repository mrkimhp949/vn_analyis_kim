...
class WalkForwardTester:
    ...
    def _aggregate_results(self, results: List[WalkForwardResult]) -> Dict:
        ...
        # Consistency metrics
        positive_windows_pct = (profitable_windows / total_windows * 100) if total_windows > 0 else 0
    ...
    def format_report(self, results: Dict) -> str:
        ...
        lines.append(f"• Windows có lời: {results['profitable_windows']} ({results['positive_windows_pct']:.1f}%)")
        lines.append(f"• Return trung bình: {results['avg_return']:.2f}% ± {results['std_return']:.2f}%")
