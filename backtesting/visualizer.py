...
class BacktestVisualizer:
    ...
    def plot_equity_curve(result: BacktestResult, save_path: Optional[str] = None):
        ...
        # Plot 1: Equity curve
        ax1.plot(dates, equity_df['equity'], linewidth=2, label='Portfolio Value', color='#2E86AB')
        ax1.axhline(y=result.initial_capital, color='gray', linestyle='--', alpha=0.5, label='Initial Capital')
        ax1.fill_between(dates, result.initial_capital, equity_df['equity'],
    ...
    def plot_trade_analysis(result: BacktestResult, save_path: Optional[str] = None):
        ...
        ax2.pie(win_loss_data, labels=labels, colors=colors_pie, autopct='%1.1f%%',
        ...
        ax3.axvline(x=np.mean(holding_days), color='red', linestyle='--',
                   linewidth=2, label=f'Mean: {np.mean(holding_days):.1f} days')
        ...
        ax4.axvline(x=np.mean(pnl_pcts), color='red', linestyle='--',
                   linewidth=2, label=f'Mean: {np.mean(pnl_pcts):.2f}%')
