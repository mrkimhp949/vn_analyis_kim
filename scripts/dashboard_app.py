...
if page == "Overview":
    ...
    with col2:
        st.metric("P&L %", f"{portfolio['pnl_percent']:+.2f}%", delta=None)

    with col3:
        st.metric("Active Positions", portfolio["num_positions"])

    with col4:
        cash = 100_000_000 - portfolio["total_cost"]
        st.metric("Cash Available", f"{cash:,.0f} VNĐ")
    ...
    with col2:
        ...
        if trades:
            df_trades = pd.DataFrame(
                trades, columns=["Symbol", "Action", "Shares", "Price", "Date"]
            )
...
# ============================================================================
    ...
    if signals:
        ...
        df_signals = pd.DataFrame(
            signals, columns=["Symbol", "Signal", "Confidence", "Reason", "Date"]
        )
...
# ============================================================================
    ...
    if trades:
        ...
        st.dataframe(
            df_trades[["symbol", "action", "shares", "price", "pnl", "date"]],
