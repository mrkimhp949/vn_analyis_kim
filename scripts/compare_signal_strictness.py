...
def simulate_signals_with_config(config_name: str, settings: dict):
    ...
    print(
        f"  2. After confidence filter:   {passed_confidence:3d} ({passed_confidence/total_candidates*100:5.1f}%)"
    )
    print(
        f"  3. After regime filter:       {passed_regime:3d} ({passed_regime/total_candidates*100:5.1f}%)"
    )
    print(
        f"  4. After timing filter:       {passed_timing:3d} ({passed_timing/total_candidates*100:5.1f}%)"
    )
    print(
        f"  5. After volume filter:       {passed_volume:3d} ({passed_volume/total_candidates*100:5.1f}%)"
    )
    print(
        f"  6. After portfolio risk:      {passed_risk:3d} ({passed_risk/total_candidates*100:5.1f}%)"
    )
    print(
...
def main():
    ...
    print(
        f"{'Configuration':<25} {'Signals %':<12} {'Trades/Mo':<12} {'Win Rate':<12} {'Quality':<10}"
    )
    print("-" * 80)
    for _, row in comparison_df.iterrows():
        print(
            f"{row['config']:<25} {row['final_signals_pct']:>10.1%} {row['estimated_trades_month']:>11.0f} {row['win_rate']:>11.1%} {row['quality_score']:>9.0f}/10"
        )
