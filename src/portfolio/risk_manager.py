...
class PortfolioRiskManager:
    ...
    def calculate_portfolio_risk(self, positions: Dict) -> PortfolioRiskMetrics:
        ...
        portfolio_risk_percent = (
            (total_risk / self.total_capital) * 100 if self.total_capital > 0 else 0
        )
    ...
    def check_circuit_breaker(self, current_portfolio_value: float) -> Tuple[bool, str]:
        ...
        if drawdown_percent >= self.max_drawdown * 100:
            return (
                True,
                f"🚨 CIRCUIT BREAKER: Drawdown {drawdown_percent:.1f}% >= {self.max_drawdown*100:.1f}%",
            )

        return False, f"✅ Drawdown OK: {drawdown_percent:.1f}%"

    def can_add_position(
        self, symbol: str, position_value: float, position_risk: float, positions: Dict
    ) -> Tuple[bool, str]:
        """
        ...
        if new_risk_percent > self.max_portfolio_risk * 100:
            return (
                False,
                f"Portfolio risk would exceed limit: {new_risk_percent:.1f}% > {self.max_portfolio_risk*100:.1f}%",
            )
        ...
        if new_sector_exp > self.max_sector_exposure * 100:
            return (
                False,
                f"Sector {sector} exposure would exceed limit: {new_sector_exp:.1f}% > {self.max_sector_exposure*100:.1f}%",
            )
        ...
        if len(positions) > 0:
            ...
            same_sector_count = sum(
                1 for s in positions.keys() if get_sector_for_symbol(s) == sector
            )
            if same_sector_count >= 3:  # Max 3 positions per sector
                return (
                    False,
                    f"Too many positions in sector {sector} ({same_sector_count})",
                )
    ...
    def get_risk_summary(self, positions: Dict) -> str:
        ...
        lines.append(
            f"⚠️ Total Risk: {metrics.total_risk_amount:,.0f} VNĐ ({metrics.portfolio_risk_percent:.1f}%)"
        )
