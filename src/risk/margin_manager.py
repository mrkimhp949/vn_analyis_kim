...
class MarginManager:
    ...
    def _generate_warnings(self, equity_ratio: float, margin_utilization: float) -> List[str]:
        ...
        if equity_ratio < self.force_liquidation_level:
            ...
            warnings.append(
                f"⚠️ MARGIN CALL: Deposit funds or reduce positions! Equity: {equity_ratio:.1%}"
            )
        elif equity_ratio < self.maintenance_margin:
        ...
        with self._lock:
            ...
            if use_margin:
                ...
                if required_cash > self.cash_balance:
                    return (
                        False,
                        f"Insufficient cash. Need: {required_cash:,.0f}, Have: {self.cash_balance:,.0f}",
                        {},
                    )
                ...
                if new_borrowed > self.margin_limit:
                    return (
                        False,
                        f"Exceeds margin limit. Need: {new_borrowed:,.0f}, Limit: {self.margin_limit:,.0f}",
                        {},
                    )
                ...
                if new_equity_ratio < self.maintenance_margin:
                    return (
                        False,
                        f"Would breach maintenance margin. Post-trade equity: {new_equity_ratio:.1%}",
                        {},
                    )
                ...
                if position_value > self.cash_balance:
                    return (
                        False,
                        f"Insufficient cash. Need: {position_value:,.0f}, Have: {self.cash_balance:,.0f}",
                        {},
                    )
    ...
    def open_position(
        ...
        with self._lock:
            ...
            if symbol in self._positions:
                ...
                new_margin_ratio = (existing.borrowed_amount + borrow_amount) / (
                    existing.market_value + position_value
                )
    ...
    def close_position(
        ...
        with self._lock:
            ...
            if quantity > position.quantity:
                return (
                    False,
                    f"Insufficient shares. Have: {position.quantity}, Want: {quantity}",
                    0.0,
                )
            ...
            if quantity == position.quantity:
                ...
                new_margin_ratio = (
                    remaining_borrowed / (remaining_qty * position.avg_cost)
                    if remaining_qty > 0
                    else 0
                )
    ...
    def simulate_price_change(
        ...
        with self._lock:
            ...
            if status == MarginStatus.FORCE_LIQUIDATION:
                ...
                actions.append(
                    f"🚨 FORCE LIQUIDATION: Sell positions worth {equity_shortfall:,.0f} VND"
                )
                ...
                actions.append(
                    f"⚠️ MARGIN CALL: Deposit {deposit_required:,.0f} VND or reduce positions"
                )
            ...
            simulated_state = MarginAccountState(
                ...
                equity_ratio=equity_ratio,
                margin_utilization=(
                    total_borrowed / self.margin_limit if self.margin_limit > 0 else 0
                ),
                buying_power=0,  # Not relevant for simulation
                status=status,
                margin_call_active=status
                in [MarginStatus.MARGIN_CALL, MarginStatus.FORCE_LIQUIDATION],
                initial_margin_req=self.initial_margin,
                maintenance_margin_req=self.maintenance_margin,
    ...
    def _suggest_liquidation_order(
        ...
        for pos in sorted_positions:
            ...
            if sell_qty > 0:
                suggestions.append(
                    f"   - Sell {pos.symbol}: {sell_qty} shares ({sell_qty * pos.current_price:,.0f} VND)"
                )
    ...
    def check_margin_call(self) -> Optional[MarginCallEvent]:
        ...
        with self._lock:
            ...
            if self._active_margin_call and state.status in [
    ...
    def execute_force_liquidation(
        ...
        with self._lock:
            ...
            for pos in sorted_positions:
                ...
                if shares_to_sell > 0:
                    success, msg, pnl = self.close_position(
                        pos.symbol, shares_to_sell, current_price
                    )
                    if success:
            ...
            if self._active_margin_call:
                self._active_margin_call.resolved = True
                self._active_margin_call.resolution_time = datetime.now()
                self._active_margin_call.resolution_action = (
                    f"Force liquidated {len(liquidations)} positions"
                )
                self._active_margin_call = None
