...
class CircuitBreakerDB:
    ...
    def _load_from_db(self) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(
                "SELECT * FROM circuit_breaker_daily WHERE date = ?",
                (today,)
            )
            row = cursor.fetchone()
    ...
    def acquire_lock(
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                    "SELECT * FROM circuit_breaker_locks WHERE lock_name = ?",
                    (lock_name,)
                )
                ...
                if existing:
                    ...
                    if expires > now and existing["bot_id"] != bot_id:
                        logger.warning(
                            f"Lock '{lock_name}' held by {existing['bot_id']} "
                            f"until {expires}"
                        )
                        return False
    ...
    def release_lock(self, lock_name: str = "trading", bot_id: str = None) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                if bot_id:
                    cursor.execute(
                        "DELETE FROM circuit_breaker_locks "
                        "WHERE lock_name = ? AND bot_id = ?",
                        (lock_name, bot_id)
                    )
                else:
                    cursor.execute(
                        "DELETE FROM circuit_breaker_locks WHERE lock_name = ?",
                        (lock_name,)
                    )
...
def get_regime_aware_loss_limit(regime: str) -> int:
    ...
    return REGIME_CONSECUTIVE_LOSS_LIMITS.get(
        regime.upper(),
        REGIME_CONSECUTIVE_LOSS_LIMITS["DEFAULT"]
    )
