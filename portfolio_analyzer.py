...
class PortfolioAnalyzer:
    ...
    def _analyze_single_stock(self, symbol, holding, market_regime):
        ...
        try:
            ...
            exit_decision = self.exit_strategy.check_exit(
                ...
                take_profit_targets=[entry_price * 1.10, entry_price * 1.15, entry_price * 1.25],
            ...
            return {
                ...
                'exit_reason': exit_decision.exit_reason.value if exit_decision.exit_reason else None,
                'exit_type': exit_decision.exit_type,
                'exit_urgency': exit_decision.urgency,
                'exit_message': exit_decision.message,
                'recommendation': recommendation,
                'recommendation_reason': self._get_recommendation_reason(recommendation, exit_decision, pnl_percent)
            }
    ...
    def _find_new_buy_opportunities(self, current_holdings, market_regime, max_recommendations=20):
        ...
        try:
            ...
            safe_print(f"   📦 Mã đang nắm: {len(current_holdings)} - {list(current_holdings.keys())}")
            ...
            for i, symbol in enumerate(symbols_to_scan):
                try:
                    ...
                    if entry_signal.should_enter:
                        ...
                        safe_print(f"       ✅ {symbol}: BUY ({entry_signal.confidence}%) - {entry_signal.strength.name}")
                    else:
                        safe_print(f"       ❌ {symbol}: {entry_signal.signal_type} ({entry_signal.confidence}%)")
                            
                except Exception as e:
                    safe_print(f"       💥 {symbol}: Lỗi - {str(e)[:50]}...")
            
            safe_print(f"🎯 KẾT QUẢ: Tìm thấy {len(buy_opportunities)}/{len(symbols_to_scan)} mã BUY từ config")
    ...
    def _calculate_portfolio_summary(self, current_holdings, market_regime, current_cash=0.0):
        ...
        total_invested = sum(h['entry_value'] for h in current_holdings.values() if 'entry_value' in h)
