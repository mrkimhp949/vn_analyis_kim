...
class PerformanceAttributionAnalyzer:
    ...
    def analyze_full_attribution(self, days: int = 90) -> Dict:
        ...
        if not trades:
            return {
                'total_trades': 0,
                'message': 'Chưa có giao dịch nào để phân tích'
            }
    ...
    def _calculate_trade_pnl(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol in df['symbol'].unique():
            ...
            for _, sell in sell_trades.iterrows():
                # Find matching buy
                matching_buys = buy_trades[buy_trades['trade_date'] < sell['trade_date']]
    ...
    def _analyze_by_sector(self, df: pd.DataFrame) -> Dict:
        ...
        for sector in df['sector'].unique():
            ...
            sector_stats[sector] = {
                ...
                'win_rate': float((sector_df['pnl'] > 0).sum() / len(sector_df) * 100) if len(sector_df) > 0 else 0,
                'best_stock': sector_df.groupby('symbol')['pnl'].sum().idxmax() if len(sector_df) > 0 else None
            }
        
        # Sort by total PnL
        sector_stats = dict(sorted(sector_stats.items(), key=lambda x: x[1]['total_pnl'], reverse=True))
    ...
    def _analyze_by_signal_strength(self, df: pd.DataFrame) -> Dict:
        ...
        for strength in df['signal_strength'].unique():
            ...
            strength_stats[strength] = {
                ...
                'win_rate': float((strength_df['pnl'] > 0).sum() / len(strength_df) * 100) if len(strength_df) > 0 else 0
            }
    ...
    def _analyze_by_market_regime(self, df: pd.DataFrame) -> Dict:
        ...
        for regime in df['market_regime'].unique():
            ...
            regime_stats[regime] = {
                ...
                'win_rate': float((regime_df['pnl'] > 0).sum() / len(regime_df) * 100) if len(regime_df) > 0 else 0
            }
    ...
    def _analyze_by_hold_period(self, df: pd.DataFrame) -> Dict:
        ...
        df['hold_category'] = pd.cut(
            ...
            labels=['0-3 days', '3-7 days', '7-14 days', '14-30 days', '30-60 days', '60+ days']
        )
        ...
        for category in df['hold_category'].unique():
            ...
            hold_stats[str(category)] = {
                ...
                'win_rate': float((category_df['pnl'] > 0).sum() / len(category_df) * 100) if len(category_df) > 0 else 0
            }
    ...
    def _analyze_by_entry_time(self, df: pd.DataFrame) -> Dict:
        ...
        df['entry_hour'] = pd.to_datetime(df['trade_date'], format='mixed', errors='coerce').dt.hour
        ...
        for session in df['session'].unique():
            ...
            session_stats[session] = {
                ...
                'win_rate': float((session_df['pnl'] > 0).sum() / len(session_df) * 100) if len(session_df) > 0 else 0
            }
    ...
    def _analyze_by_month(self, df: pd.DataFrame) -> Dict:
        ...
        df['month'] = pd.to_datetime(df['trade_date'], format='mixed', errors='coerce').dt.to_period('M')
        ...
        for month in df['month'].unique():
            ...
            month_stats[str(month)] = {
                ...
                'win_rate': float((month_df['pnl'] > 0).sum() / len(month_df) * 100) if len(month_df) > 0 else 0
            }
        
        return month_stats
    
    def _get_top_trades(self, df: pd.DataFrame, top_n: int = 5, best: bool = True) -> List[Dict]:
        ...
        return [
            {
                'symbol': row['symbol'],
                'pnl': float(row['pnl']),
                'pnl_percent': float((row['pnl'] / row['total_value']) * 100) if row['total_value'] > 0 else 0,
    ...
    def format_attribution_report(self, attribution: Dict) -> str:
        ...
        if attribution.get('by_sector'):
            ...
            for sector, stats in list(attribution['by_sector'].items())[:5]:
                ...
                lines.append(f"  └ P&L: {stats['total_pnl']:+,.0f} VNĐ | Win rate: {stats['win_rate']:.1f}% | {stats['total_trades']} trades")
            lines.append("")
        ...
        if attribution.get('by_signal_strength'):
            ...
            for strength, stats in attribution['by_signal_strength'].items():
                ...
                lines.append(f"  └ P&L: {stats['total_pnl']:+,.0f} VNĐ | Win rate: {stats['win_rate']:.1f}%")
            lines.append("")
        ...
        if attribution.get('top_winners'):
            ...
            for trade in attribution['top_winners']:
                lines.append(f"• {trade['symbol']}: {trade['pnl']:+,.0f} VNĐ ({trade['pnl_percent']:+.1f}%) - {trade['hold_days']} days")
            lines.append("")
