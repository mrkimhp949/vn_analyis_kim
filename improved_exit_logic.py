...
class ImprovedExitStrategy:
    ...
    def __init__(self,
                 ...
                 trailing_stop_distance_atr: float = 1.5, # Trailing 1.5x ATR từ đỉnh
        ...
        self.trailing_distance_atr = trailing_stop_distance_atr
    ...
    def check_exit(self,
        ...
        trailing_check = self._check_trailing_stop(
            ...
            pnl_percent,
            pnl_amount,
            df.iloc[-1].get('atr', current_price * 0.02) # Pass ATR
        )
    ...
    def _check_take_profit_levels(self,
                                  current_price: float,
                                  tp_targets: List[float],
                                  partial_exits: List[float],
                                  pnl_percent: float,
                                  pnl_amount: float) -> Dict:
        """
    ...
    def _check_trailing_stop(self,
                            ...
                            atr: float) -> Dict:
        """
        Check trailing stop based on ATR.
        Kích hoạt khi lời >= 8%
        Trailing stop cách đỉnh 1.5 * ATR.
        """
        ...
        trailing_stop_price = highest_price - (atr * self.trailing_distance_atr)
        ...
        if current_price <= trailing_stop_price:
            ...
            return {
                ...
                'decision': ExitDecision(
                    ...
                    expected_pnl=pnl_amount,
                    expected_pnl_percent=pnl_percent,
                    message=f"📉 TRAILING STOP (ATR): Giảm {drawdown_from_high:.1f}% từ đỉnh | "
    ...
    def _check_reversal_pattern(self,
                               df: pd.DataFrame,
                               pnl_percent: float,
                               pnl_amount: float) -> Dict:
        """
    ...
    def _check_support_breakdown(self,
                                df: pd.DataFrame,
                                current_price: float,
                                entry_price: float,
                                pnl_percent: float,
