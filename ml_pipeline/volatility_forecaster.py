...
class VolatilityForecaster:
    ...
    def _calculate_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'close' in df.columns:
            ...
            features_df['volatility_ratio'] = features_df['volatility_5d'] / (features_df['volatility_20d'] + 1e-6)
        ...
        if all(col in df.columns for col in ['high', 'low', 'close']):
            ...
            features_df['range_ma_ratio'] = features_df['daily_range'] / (features_df['daily_range'].rolling(20).mean() + 1e-6)
    ...
    def _prepare_target(self, df: pd.DataFrame) -> pd.Series:
        ...
        future_volatility = returns.shift(-self.forecast_horizon).rolling(self.forecast_horizon).std()
    ...
    def train(self, df: pd.DataFrame) -> Dict:
        ...
        feature_cols = [
            ...
            'atr_pct', 'atr_ma_ratio',
            'volume_ratio', 'volume_volatility',
            'daily_range', 'range_ma_ratio',
            'rsi_volatility', 'macd_volatility',
        ]
        ...
        logger.info(f"Volatility model trained - Test MAE: {metrics['test_mae']:.6f}, RMSE: {metrics['test_rmse']:.6f}")
    ...
    def forecast(self, df: pd.DataFrame) -> float:
        ...
        if len(available_features) < 3:
            ...
            if 'atr' in df.columns and 'close' in df.columns:
                return (df['atr'].iloc[-1] / df['close'].iloc[-1]) if df['close'].iloc[-1] > 0 else 0.02
            return 0.02
