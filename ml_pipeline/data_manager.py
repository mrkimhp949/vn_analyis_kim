...
class DataManager:
    ...
    def ingest_symbol(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        if os.path.exists(feature_path) and not self.config.refresh:
            try:
                ...
                logger.warning(f"Failed to load cached features for {symbol}: {e}, will reload")
    ...
    def ingest_all(self) -> pd.DataFrame:
        ...
        logger.info(f"Starting ingestion for {len(self.config.tickers)} tickers: {self.config.tickers}")
        ...
        logger.info(f"✅ Combined dataset saved: {len(combined)} rows, {len(combined.columns)} columns")
