    ...
    def format_dashboard_report(self, dashboard: Dict) -> str:
        ...
        if perf.get('status') == 'success' and perf.get('data'):
            ...
            if by_sector:
                ...
                lines.append(f"• Best sector: {top_sector[0]} ({top_sector[1]['total_pnl']:+,.0f} VNĐ)")
            lines.append("")
        ...
        if portfolio.get('status') == 'success' and portfolio.get('data'):
            ...
            lines.append(f"• Total P&L: {data.get('total_pnl', 0):+,.0f} VNĐ ({data.get('total_return_pct', 0):+.1f}%)")
            lines.append("")
